...
namespace {
struct Signal { std::string id, category, weight, title, detail; };
...
bool containsAny(const std::string& value, const std::vector<std::string>& needles) {
    ...
    return std::any_of(needles.begin(), needles.end(), [&](const auto& needle) { return lower.find(needle) != std::string::npos; });
}
...
std::vector<Signal> collect() {
    ...
    for (const char* path : rootPaths) {
        if (rawExists(path)) { add(out, "root_artifact", "ROOT", 40, "Privileged artifact", path); break; }
    }
    ...
    while (start < maps.size()) {
        ...
        if (line.find("rwxp") != std::string::npos && line.find('/') == std::string::npos) { anonymousExecutable = true; break; }
...
Java_com_example_devicetrust_detection_NativeSignalSource_collectEncoded(JNIEnv* env, jobject) {
